...
const OverallHistory = () => {
  ...
  const loadTransactions = async () => {
    try {
      const data = await fetchAllHistoryTransactions();
                      ...
                      {sortTransactions(transactions).map((transaction) => {
                        ...
                        return (
                          <tr key={transaction.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className={`py-2 px-4 font-medium ${transaction.type === 'IN' ? 'text-green-600' : 'text-red-600'}`}>
                              {transaction.type}
                            </td>
                            <td className="py-2 px-4">{transaction.quantity}</td>
                            <td className="py-2 px-4">{suppliers[transaction.supplier_id] || 'N/A'}</td>
                            <td className="py-2 px-4">{stock ? stock.model_name : 'N/A'}</td>
                            <td className="py-2 px-4">{stock ? stock.brand_name : 'N/A'}</td>
