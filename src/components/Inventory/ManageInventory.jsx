...
const ManageInventory = () => {
  ...
  const [outOfStockCount, setOutOfStockCount] = useState(0); // New state for "Out of Stock" count
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      try {
        ...
        const outOfStockItems = transformedStocks.filter((stock) => stock.quantityStock === 0).length;
        setOutOfStockCount(outOfStockItems);
  
        // Calculate "Low Stock" count (0 < quantity <= 10)
        const lowStocks = transformedStocks.filter(
          (stock) => stock.quantityStock > 0 && stock.quantityStock <= 10
        ).length;
    ...
    const fetchTotalStocks = async () => {
      try {
        const totalStocksData = await getTotalStocks();
        setTotalStocks(totalStocksData.total);
      } catch (error) {
        console.error("Error fetching total stocks:", error);
      }
    };
  
    // Fetch stocks and total stocks simultaneously
    fetchStocks();
    fetchTotalStocks();
  }, []);
  ...
  <h2 className="text-xl font-semibold text-gray-700">
    Low Stocks:{" "}
    <span className="text-red-500 text-2xl font-bold">{lowStockCount}</span>
  </h2>
  <h2 className="text-xl font-semibold text-gray-700">
    Out of Stock:{" "}
    <span className="text-red-900 text-2xl font-bold">{outOfStockCount}</span>
  </h2>
</div>
      </div>
